				...
				BM_BUF				*pBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "AllocMemory (pBMContext=%p, uSize=0x%x, uFlags=0x%x, align=0x%x, pBuf=%p)",
			  pBMContext, (unsigned int)uSize, (unsigned int)uFlags, (unsigned int)uDevVAddrAlignment, pBuf));
	...
	if(uFlags & PVRSRV_MEM_RAM_BACKED_ALLOCATION)
	{
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "AllocMemory: RA_Alloc(0x%x) FAILED", (unsigned int)uSize));
			return IMG_FALSE;
		}
	...
	else
	{
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "AllocMemory: OSAllocMem(0x%x) FAILED", sizeof (struct _BM_MAPPING_)));
			return IMG_FALSE;
		}
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
				"AllocMemory: pMapping=%p: DevV=%08X CpuV=%08X CpuP=%08X uSize=0x%x",
				pMapping,
				(unsigned int)pMapping->DevVAddr.uiAddr,
				(unsigned int)pMapping->CpuVAddr,
				(unsigned int)pMapping->CpuPAddr.uiAddr,
				(unsigned int)pMapping->uSize));

	PVR_DPF ((PVR_DBG_MESSAGE,
				"AllocMemory: pBuf=%p: DevV=%08X CpuV=%08X CpuP=%08X uSize=0x%x",
				pBuf,
				(unsigned int)pBuf->DevVAddr.uiAddr,
				(unsigned int)pBuf->CpuVAddr,
				(unsigned int)pBuf->CpuPAddr.uiAddr,
				(unsigned int)uSize));
			...
			BM_BUF *pBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "WrapMemory(psBMHeap=%p, size=0x%x, offset=0x%x, bPhysContig=0x%x, pvCPUVAddr = 0x%p, flags=0x%x, pBuf=%p)",
			  psBMHeap, (unsigned int)uSize, (unsigned int)ui32BaseOffset, bPhysContig, pvCPUVAddr, (unsigned int)uFlags, pBuf));
	...
	if(pvCPUVAddr)
	{
		...
		if (bPhysContig)
		{
			...
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSRegisterMem Phys=0x%08X, CpuVAddr = 0x%08X, Size=%u) failed",
					(unsigned int)pMapping->CpuPAddr.uiAddr, (unsigned int)pMapping->CpuVAddr, (unsigned int)pMapping->uSize));
				goto fail_cleanup;
			}
		}
		else
		{
			...
			if(OSRegisterDiscontigMem(pMapping->psSysAddr, 
							...
							&pMapping->hOSMemHandle) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSRegisterDiscontigMem CpuVAddr = 0x%08X, Size=%u) failed",
					(unsigned int)pMapping->CpuVAddr, (unsigned int)pMapping->uSize));
				goto fail_cleanup;
			}
		}
	}
	else
	{
		if (bPhysContig)
		{
			...
			if(OSReservePhys(pMapping->CpuPAddr,
							 ...
							 &pMapping->hOSMemHandle) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSReservePhys Phys=0x%08X, Size=%u) failed",
					(unsigned int)pMapping->CpuPAddr.uiAddr, (unsigned int)pMapping->uSize));
				goto fail_cleanup;
			}
		}
		else
		{
			...
			if(OSReserveDiscontigPhys(pMapping->psSysAddr,
							 ...
							 &pMapping->hOSMemHandle) != PVRSRV_OK)
			{
				PVR_DPF((PVR_DBG_ERROR,	"WrapMemory: OSReserveDiscontigPhys Size=%u) failed",
					(unsigned int)pMapping->uSize));
				goto fail_cleanup;
			}
		}
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "DevVaddr.uiAddr=%08X", (unsigned int)DevVAddr.uiAddr));
	PVR_DPF ((PVR_DBG_MESSAGE,
				"WrapMemory: pMapping=%p: DevV=%08X CpuV=%08X CpuP=%08X uSize=0x%x",
				pMapping, (unsigned int)pMapping->DevVAddr.uiAddr,
				(unsigned int)pMapping->CpuVAddr, (unsigned int)pMapping->CpuPAddr.uiAddr, (unsigned int)pMapping->uSize));
	PVR_DPF ((PVR_DBG_MESSAGE,
				"WrapMemory: pBuf=%p: DevV=%08X CpuV=%08X CpuP=%08X uSize=0x%x",
				pBuf, (unsigned int)pBuf->DevVAddr.uiAddr,
				(unsigned int)pBuf->CpuVAddr, (unsigned int)pBuf->CpuPAddr.uiAddr, (unsigned int)uSize));
...
FreeBuf (BM_BUF *pBuf, IMG_UINT32 ui32Flags, IMG_BOOL bFromAllocator)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			"FreeBuf: pBuf=%p DevVAddr=%08X CpuVAddr=%08X CpuPAddr=%08X",
			pBuf, (unsigned int)pBuf->DevVAddr.uiAddr, (unsigned int)pBuf->CpuVAddr, (unsigned int)pBuf->CpuPAddr.uiAddr));
			...
			BM_HANDLE			*phBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "BM_Alloc (uSize=0x%x, uFlags=0x%x, uDevVAddrAlignment=0x%x)",
			(unsigned int)uSize, (unsigned int)uFlags, (unsigned int)uDevVAddrAlignment));
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "BM_Alloc (uSize=0x%x, uFlags=0x%x)=%p",
		  (unsigned int)uSize, (unsigned int)uFlags, pBuf));
			...
			BM_HANDLE *phBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "BM_Wrap (uSize=0x%x, uOffset=0x%x, bPhysContig=0x%x, pvCPUVAddr=0x%p, uFlags=0x%x)",
			(unsigned int)ui32Size, (unsigned int)ui32Offset, bPhysContig, pvCPUVAddr, (unsigned int)uFlags));
	...
	if(pBuf)
	{
														...
														pBuf->pMapping->eCpuMemoryOrigin == hm_wrapped_virtaddr))
		{
			PVR_DPF((PVR_DBG_MESSAGE,
					...
					(unsigned int)ui32Size, (unsigned int)ui32Offset, (unsigned int)sHashAddress.uiAddr));
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			"BM_Wrap (uSize=0x%x, uFlags=0x%x)=%p(devVAddr=%08X)",
			(unsigned int)ui32Size, (unsigned int)uFlags, pBuf, (unsigned int)pBuf->DevVAddr.uiAddr));
		...
		IMG_UINT32 ui32Flags)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "BM_Free (h=%p)", hBuf));
...
BM_HandleToCpuVaddr (BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
				"BM_HandleToCpuVaddr(h=%p)=%08X",
				hBuf, (unsigned int)pBuf->CpuVAddr));
...
BM_HandleToDevVaddr (BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "BM_HandleToDevVaddr(h=%p)=%08X", hBuf, (unsigned int)pBuf->DevVAddr.uiAddr));
...
BM_HandleToSysPaddr (BM_HANDLE hBuf)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "BM_HandleToSysPaddr(h=%p)=%08X", hBuf, (unsigned int)pBuf->CpuPAddr.uiAddr));
			  ...
			  IMG_UINTPTR_T *pBase)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "BM_ImportMemory (pBMContext=%p, uRequestSize=0x%x, uFlags=0x%x, uAlign=0x%x)",
			  pBMContext, (unsigned int)uRequestSize, (unsigned int)uFlags, (unsigned int)uDevVAddrAlignment));
	...
	else if(pBMHeap->ui32Attribs & PVRSRV_BACKINGSTORE_LOCALMEM_CONTIG)
	{
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "BM_ImportMemory: RA_Alloc(0x%x) FAILED", (unsigned int)uPSize));
			goto fail_mapping_alloc;
		}
...
BM_FreeMemory (IMG_VOID *h, IMG_UINTPTR_T _base, BM_MAPPING *psMapping)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "BM_FreeMemory (h=%p, base=0x%x, psMapping=0x%x)", h, (unsigned int)_base, (unsigned int)psMapping));
	...
	PVR_DPF((PVR_DBG_MESSAGE,
			"..BM_FreeMemory (h=%p, base=0x%x, psMapping=0x%x)",
			h, (unsigned int)_base, (unsigned int)psMapping));
}
