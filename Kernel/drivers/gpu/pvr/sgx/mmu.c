						  ...
						  IMG_HANDLE hUniqueTag)
{
	...
	for(i=0; i<ui32PageCount; i++)
	{
...

		{
			...
			if (!ppsPTInfoList[0])
			{
				PVR_DPF((PVR_DBG_MESSAGE, "MMU_UnmapPagesAndFreePTs: Invalid PT for alloc at VAddr:0x%08lX (VaddrIni:0x%08lX AllocPage:%u) PDIdx:%u PTIdx:%u",sTmpDevVAddr.uiAddr, sDevVAddr.uiAddr, (unsigned int)i, (unsigned int)ui32PDIndex, (unsigned int)ui32PTIndex ));
			...
			{
				PVR_DPF((PVR_DBG_MESSAGE, "MMU_UnmapPagesAndFreePTs: Page is already invalid for alloc at VAddr:0x%08lX (VAddrIni:0x%08lX AllocPage:%u) PDIdx:%u PTIdx:%u",sTmpDevVAddr.uiAddr, sDevVAddr.uiAddr, (unsigned int)i, (unsigned int)ui32PDIndex, (unsigned int)ui32PTIndex ));
			}
...
MMU_Free (MMU_HEAP *pMMUHeap, IMG_DEV_VIRTADDR DevVAddr, IMG_UINT32 ui32Size)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		"MMU_Free: mmu=%p, dev_vaddr=%08X", pMMUHeap, (unsigned int)DevVAddr.uiAddr));
	...
	PVR_DPF((PVR_DBG_ERROR,"MMU_Free: Couldn't find DevVAddr %08X in a DevArena", (unsigned int)DevVAddr.uiAddr));
}
			 ...
			 IMG_UINT32 ui32MemFlags)
{
	...
	if (pui32Tmp[ui32Index] & SGX_MMU_PTE_VALID)
	{
		PVR_DPF((PVR_DBG_ERROR, "MMU_MapPage: Page is already valid for alloc at VAddr:0x%08lX PDIdx:%u PTIdx:%u",
								DevVAddr.uiAddr,
								(unsigned int)DevVAddr.uiAddr >> pMMUHeap->ui32PDShift,
	...
	for (i=0, uCount=0; uCount<uSize; i++, uCount+=pMMUHeap->ui32DataPageSize)
	{
		...
		PVR_DPF ((PVR_DBG_MESSAGE,
				 "MMU_MapScatter: devVAddr=%08X, SysAddr=%08X, size=0x%x/0x%x",
				  (unsigned int)DevVAddr.uiAddr, (unsigned int)sSysAddr.uiAddr, (unsigned int)uCount, (unsigned int)uSize));
	}
			  ...
			  IMG_HANDLE hUniqueTag)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		  "MMU_MapPages: mmu=%p, devVAddr=%08X, SysPAddr=%08X, size=0x%x",
		  pMMUHeap, (unsigned int)DevVAddr.uiAddr, (unsigned int)SysPAddr.uiAddr, (unsigned int)uSize));
			   ...
			   IMG_HANDLE         hUniqueTag)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			...
			(unsigned int)MapBaseDevVAddr.uiAddr,
			(unsigned int)uByteSize,
			(unsigned int)CpuVAddr));
	...
	for (i=0; i<uByteSize; i+=ui32VAdvance)
	{
		...
		PVR_DPF ((PVR_DBG_MESSAGE,
				...
				(unsigned int)(CpuVAddr + uOffset),
				(unsigned int)CpuPAddr.uiAddr,
				(unsigned int)MapDevVAddr.uiAddr,
				(unsigned int)DevPAddr.uiAddr));
	...
	for(i=0; i<ui32PageCount; i++)
	{
		...
		if (!ppsPTInfoList[0])
		{
			PVR_DPF((PVR_DBG_ERROR, "MMU_UnmapPages: ERROR Invalid PT for alloc at VAddr:0x%08lX (VaddrIni:0x%08lX AllocPage:%u) PDIdx:%u PTIdx:%u",
									...
									sDevVAddr.uiAddr,
									(unsigned int)i,
									(unsigned int)ui32PDIndex,
									(unsigned int)ui32PTIndex));
...
MMU_GetPhysPageAddr(MMU_HEAP *pMMUHeap, IMG_DEV_VIRTADDR sDevVPageAddr)
{
	...
	if (!ppsPTInfoList[0])
	{
		PVR_DPF((PVR_DBG_ERROR,"MMU_GetPhysPageAddr: Not mapped in at 0x%08x", (unsigned int)sDevVPageAddr.uiAddr));
		sDevPAddr.uiAddr = 0;
		return sDevPAddr;
	}
