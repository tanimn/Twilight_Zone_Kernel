...
{	\
	...
	{	\
		PVR_DPF((PVR_DBG_ERROR, "SGXDevInitCompatCheck: Size check failed for SGXMKIF_%s (client) = %u bytes, (ukernel) = %u bytes\n", \
			VAR(NAME), \
			(unsigned int)psDevInfo->sSGXStructSizes.ui32Sizeof_##NAME, \
			(unsigned int)psSGXStructSizes->ui32Sizeof_##NAME )); \
...
static PVRSRV_ERROR SGXRunScript(PVRSRV_SGXDEV_INFO *psDevInfo, SGX_INIT_COMMAND *psScript, IMG_UINT32 ui32NumInitCommands)
{
		...
		ui32PC++, psComm++)
	{
		switch (psComm->eOp)
		{
			...
			default:
			{
				PVR_DPF((PVR_DBG_ERROR,"SGXRunScript: PC %u: Illegal command: %d", (unsigned int)ui32PC, psComm->eOp));
						   ...
						   IMG_BOOL			  bDumpSGXRegs)
{
	...
	if (bDumpSGXRegs)
	{
		...
		PVR_DPF((PVR_DBG_ERROR,"SGX Register Base Address (Physical): 0x%08X", (unsigned int)psDevInfo->sRegsPhysBase.uiAddr));
	...
	#if defined(PVRSRV_USSE_EDM_STATUS_DEBUG)
	{
		...
		PVR_LOG(("Last SGX microkernel status code: 0x%x", (unsigned int)ui32LastStatusCode));
...

		{
				 ...
				 ui32LoopCounter++)
			{
				...
				PVR_LOG(("(MKT%u) %08X %08X %08X %08X", (unsigned int)ui32LoopCounter,
						 (unsigned int)pui32BufPtr[2], (unsigned int)pui32BufPtr[3], (unsigned int)pui32BufPtr[1], (unsigned int)pui32BufPtr[0]));
			}
		}
		#endif 
	}
...

	{
		...
		for (ui32LoopCounter = 0;
			 ...
			 ui32LoopCounter += 4)
		{
			PVR_LOG(("\t0x%X: 0x%08X 0x%08X 0x%08X 0x%08X", (unsigned int)ui32LoopCounter * sizeof(*pui32HostCtlBuffer),
					(unsigned int)pui32HostCtlBuffer[ui32LoopCounter + 0], (unsigned int)pui32HostCtlBuffer[ui32LoopCounter + 1],
					(unsigned int)pui32HostCtlBuffer[ui32LoopCounter + 2], (unsigned int)pui32HostCtlBuffer[ui32LoopCounter + 3]));
		}
	}

	{
		...
		for (ui32LoopCounter = 0;
			 ...
			 ui32LoopCounter += 4)
		{
			PVR_LOG(("\t0x%X: 0x%08X 0x%08X 0x%08X 0x%08X", (unsigned int)ui32LoopCounter * sizeof(*pui32TA3DCtlBuffer),
					(unsigned int)pui32TA3DCtlBuffer[ui32LoopCounter + 0], (unsigned int)pui32TA3DCtlBuffer[ui32LoopCounter + 1],
					(unsigned int)pui32TA3DCtlBuffer[ui32LoopCounter + 2], (unsigned int)pui32TA3DCtlBuffer[ui32LoopCounter + 3]));
		}
	}
...
IMG_VOID SGXOSTimer(IMG_VOID *pvData)
{
	...
	else
	{
			...
			(psDevInfo->ui32NumResets == ui32NumResets))
		{
			...
			{
				ui32LockupCounter = 0;
				PVR_DPF((PVR_DBG_ERROR, "SGXOSTimer() detected SGX lockup (0x%x tasks)", (unsigned int)ui32EDMTasks));
...
PVRSRV_ERROR SGXDevInitCompatCheck(PVRSRV_DEVICE_NODE *psDeviceNode)
{
	...
	{
		PVR_LOG(("(FAIL) SGXInit: Incompatible driver DDK revision (%d)/device DDK revision (%lu).",
	...
	{
		PVR_DPF((PVR_DBG_MESSAGE, "SGXInit: driver DDK (%d) and device DDK (%lu) match. [ OK ]",
				PVRVERSION_BUILD, psSGXFeatures->ui32DDKBuild));
	}
