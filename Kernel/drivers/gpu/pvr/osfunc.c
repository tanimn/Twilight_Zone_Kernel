        ...
        case PVRSRV_HAP_MULTI_PROCESS:
            if(PVRMMapRemoveRegisteredArea(psLinuxMemArea) != PVRSRV_OK)
            {
                PVR_DPF((PVR_DBG_ERROR,
                         ...
                         (unsigned int)ui32AllocFlags, ui32Bytes, pvCpuVAddr, hOSMemHandle));
                return PVRSRV_ERROR_GENERIC;
            }
            break;
        default:
            PVR_DPF((PVR_DBG_ERROR,"%s: invalid flags 0x%x\n",
                    __FUNCTION__, (unsigned int)ui32AllocFlags));
            return PVRSRV_ERROR_INVALID_PARAMS;
    }
                                    ...
                                    IMG_VOID *pvDeviceNode)
{
    ...
    {
        PVR_DPF((PVR_DBG_ERROR, "OSInstallDeviceLISR: An ISR has already been installed: IRQ %u cookie %p", (unsigned int)psEnvData->ui32IRQ, psEnvData->pvISRCookie));
        return PVRSRV_ERROR_GENERIC;
    }
    ...
    {
        PVR_DPF((PVR_DBG_ERROR,"OSInstallDeviceLISR: Couldn't install device LISR on IRQ %u", (unsigned int)ui32Irq));
...
PVRSRV_ERROR OSInstallSystemLISR(IMG_VOID *pvSysData, IMG_UINT32 ui32Irq)
{
    ...
    if (psEnvData->bLISRInstalled)
    {
        PVR_DPF((PVR_DBG_ERROR, "OSInstallSystemLISR: An LISR has already been installed: IRQ %u cookie %p", (unsigned int)psEnvData->ui32IRQ, psEnvData->pvISRCookie));
        return PVRSRV_ERROR_GENERIC;
    }
    ...
    if(request_irq(ui32Irq, SystemISRWrapper,
        ...
        , "PowerVR", pvSysData))
    {
        PVR_DPF((PVR_DBG_ERROR,"OSInstallSystemLISR: Couldn't install system LISR on IRQ %u", (unsigned int)ui32Irq));
...
PVRSRV_ERROR OSUnlockResource (PVRSRV_RESOURCE *psResource, IMG_UINT32 ui32ID)
{
    ...
    if(*pui32Access)
    {
        ...
        else
        {
            ...
            PVR_DPF((PVR_DBG_MESSAGE,"Should be %x is actually %x", (unsigned int)ui32ID, (unsigned int)psResource->ui32ID));
            eError = PVRSRV_ERROR_GENERIC;
        }
    }
    else
                ...
                IMG_HANDLE hOSMemHandle)
{
    ...
    switch(ui32MappingFlags & PVRSRV_HAP_MAPTYPE_MASK)
    {
        ...
        case PVRSRV_HAP_MULTI_PROCESS:
        {
            if(PVRMMapRemoveRegisteredArea(psLinuxMemArea) != PVRSRV_OK)
            {
                 PVR_DPF((PVR_DBG_ERROR, "%s(%p, %u, 0x%08X, %p) FAILED!",
                          __FUNCTION__, pvCpuVAddr, (unsigned int)ui32Bytes,
                          (unsigned int)ui32MappingFlags, hOSMemHandle));
                return PVRSRV_ERROR_GENERIC;
            }
            break;
        }
        default:
        {
            PVR_DPF((PVR_DBG_ERROR, "OSUnRegisterMem : invalid flags 0x%x", (unsigned int)ui32MappingFlags));
            return PVRSRV_ERROR_INVALID_PARAMS;
        }
    }
                ...
                IMG_HANDLE hOSMemHandle)
{
   ...
   PVR_UNREFERENCED_PARAMETER(ui32Bytes);
    ...
    switch(ui32MappingFlags & PVRSRV_HAP_MAPTYPE_MASK)
    {
        ...
        default:
        {
            PVR_DPF((PVR_DBG_ERROR, "OSUnMapPhysToLin : invalid flags 0x%x", (unsigned int)ui32MappingFlags));
            return PVRSRV_ERROR_INVALID_PARAMS;
        }
    }
