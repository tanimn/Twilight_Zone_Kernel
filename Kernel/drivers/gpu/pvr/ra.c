...
_InsertResourceSpan (RA_ARENA *pArena, IMG_UINTPTR_T base, IMG_SIZE_T uSize)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  ...
			  pArena->name, (unsigned int)base, (unsigned int)uSize));
		...
		if (pArena->aHeadFree[uIndex]!=IMG_NULL)
		{
			...
			while (pBT!=IMG_NULL)
			{
				...
				PVR_DPF ((PVR_DBG_MESSAGE,
						...
						(unsigned int)pBT->base, (unsigned int)pBT->uSize, (unsigned int)aligned_base, (unsigned int)uSize));
				...
				if (pBT->base + pBT->uSize >= aligned_base + uSize)
				{
					...
					{
						PVR_DPF ((PVR_DBG_MESSAGE,
								"AttemptAllocAligned: mismatch in flags. Import has %x, request was %x", (unsigned int)pBT->psMapping->ui32Flags, (unsigned int)uFlags));
		   ...
		   IMG_VOID *pImportHandle)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_Create: name='%s', base=0x%x, uSize=0x%x, alloc=0x%p, free=0x%p",
			  name, (unsigned int)base, (unsigned int)uSize, imp_alloc, imp_free));
...
RA_Delete (RA_ARENA *pArena)
{
	...
	while (pArena->pHeadSegment != IMG_NULL)
	{
		...
		if (pBT->type != btt_free)
		{
			...
			PVR_DPF ((PVR_DBG_ERROR,"RA_Delete: base = 0x%x size=0x%x", (unsigned int)pBT->base, (unsigned int)pBT->uSize));
		}
	...
	if (pArena != IMG_NULL)
	{
		while (pArena->pHeadSegment != IMG_NULL)
		{
			...
			if (pBT->type != btt_free)
			{
				...
				PVR_DPF ((PVR_DBG_ERROR,"RA_TestDelete: base = 0x%x size=0x%x", (unsigned int)pBT->base, (unsigned int)pBT->uSize));
				return IMG_FALSE;
			}
		}
	}
...
RA_Add (RA_ARENA *pArena, IMG_UINTPTR_T base, IMG_SIZE_T uSize)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_Add: name='%s', base=0x%x, size=0x%x", pArena->name, (unsigned int)base, (unsigned int)uSize));
		  ...
		  IMG_UINTPTR_T *base)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
		   ...
		   pArena->name, (unsigned int)uSize, (unsigned int)uRequestSize, (unsigned int)uAlignment, (unsigned int)uAlignmentOffset));
		...
		if (bResult)
		{
			...
			if (pBT == IMG_NULL)
			{
				...
				PVR_DPF ((PVR_DBG_MESSAGE,
						  ...
						  pArena->name, (unsigned int)uSize));
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  ...
			  pArena->name, (unsigned int)uSize, (unsigned int)(*base), bResult));
...
RA_Free (RA_ARENA *pArena, IMG_UINTPTR_T base, IMG_BOOL bFreeBackingStore)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
			  "RA_Free: name='%s', base=0x%x", pArena->name, (unsigned int)base));
