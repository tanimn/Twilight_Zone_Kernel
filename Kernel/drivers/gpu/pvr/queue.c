...
static IMG_VOID QueueDumpDebugInfo_ForEachCb(PVRSRV_DEVICE_NODE *psDeviceNode)
{
	if (psDeviceNode->sDevId.eDeviceClass == PVRSRV_DEVICE_CLASS_DISPLAY)
	{
		...
		{
			PVR_LOG(("There is no Command Complete Data for display device %u", (unsigned int)psDeviceNode->sDevId.ui32DeviceIndex));
		}
	}
}
								  ...
								  IMG_BOOL			bFlush)
{
		...
		|| (ui32ReadOpsComplete != psWalkerObj->ui32ReadOpsPending))
		{
				...
				SYNCOPS_STALE(ui32ReadOpsComplete, psWalkerObj->ui32ReadOpsPending))
			{
				PVR_DPF((PVR_DBG_WARNING,
						...
						psSyncData, (unsigned int)ui32WriteOpsComplete, (unsigned int)psWalkerObj->ui32WriteOpsPending));
			}
	...
	if (psCommand->ui32DevIndex >= SYS_DEVICE_COUNT)
	{
		PVR_DPF((PVR_DBG_ERROR,
					...
					(unsigned int)psCommand->ui32DevIndex));
										 ...
										 IMG_UINT32		ui32CmdCount)
{
	...
	for (i=0; i<ui32CmdCount; i++)
	{
		...
		if (eError != PVRSRV_OK)
		{
			PVR_DPF((PVR_DBG_ERROR,"PVRSRVRegisterCmdProcListKM: Failed to alloc cmd %u", (unsigned int)i));
			goto ErrorExit;
		}
