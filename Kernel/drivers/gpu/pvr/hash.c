...
_Resize (HASH_TABLE *pHash, IMG_UINT32 uNewSize)
{
	if (uNewSize != pHash->uSize)
    {
		...
		PVR_DPF ((PVR_DBG_MESSAGE,
				...
				(unsigned int)pHash->uSize, (unsigned int)uNewSize, (unsigned int)pHash->uCount));
...
HASH_TABLE * HASH_Create_Extended (IMG_UINT32 uInitialLen, IMG_SIZE_T uKeySize, HASH_FUNC *pfnHashFunc, HASH_KEY_COMP *pfnKeyComp)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "HASH_Create_Extended: InitialSize=0x%x", (unsigned int)uInitialLen));
...
HASH_Insert_Extended (HASH_TABLE *pHash, IMG_VOID *pKey, IMG_UINTPTR_T v)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE,
              "HASH_Insert_Extended: Hash=%p, pKey=%p, v=0x%x", pHash, pKey, (unsigned int)v));
...
{
	PVR_DPF ((PVR_DBG_MESSAGE,
              "HASH_Insert: Hash=%p, k=0x%x, v=0x%x", pHash, (unsigned int)k, (unsigned int)v));
...
HASH_Remove_Extended(HASH_TABLE *pHash, IMG_VOID *pKey)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "HASH_Remove_Extended: Hash=%p, pKey=%p", pHash, pKey));
	...
	for (ppBucket = &(pHash->ppBucketTable[uIndex]); *ppBucket != IMG_NULL; ppBucket = &((*ppBucket)->pNext))
	{
		...
		if (KEY_COMPARE(pHash, (*ppBucket)->k, pKey))
		{
			...
			PVR_DPF ((PVR_DBG_MESSAGE,
                      "HASH_Remove_Extended: Hash=%p, pKey=%p = 0x%x",
                      pHash, pKey, (unsigned int)v));
			return v;
		}
	}
	PVR_DPF ((PVR_DBG_MESSAGE,
              "HASH_Remove_Extended: Hash=%p, pKey=%p = 0x0 !!!!", pHash, pKey));
	return 0;
}
...
{
	PVR_DPF ((PVR_DBG_MESSAGE, "HASH_Remove: Hash=%p, k=0x%x", pHash, (unsigned int)k));
...
HASH_Retrieve_Extended (HASH_TABLE *pHash, IMG_VOID *pKey)
{
	...
	PVR_DPF ((PVR_DBG_MESSAGE, "HASH_Retrieve_Extended: Hash=%p, pKey=%p", pHash,pKey));
	...
	for (ppBucket = &(pHash->ppBucketTable[uIndex]); *ppBucket != IMG_NULL; ppBucket = &((*ppBucket)->pNext))
	{
		...
		{         
			...
			PVR_DPF ((PVR_DBG_MESSAGE,
                      "HASH_Retrieve: Hash=%p, pKey=%p = 0x%x",
                      pHash, pKey, (unsigned int)v));
			return v;
		}
	}
	PVR_DPF ((PVR_DBG_MESSAGE,
              "HASH_Retrieve: Hash=%p, pKey=%p = 0x0 !!!!", pHash, pKey));
	return 0;
}
...
{
	PVR_DPF ((PVR_DBG_MESSAGE, "HASH_Retrieve: Hash=%p, k=0x%x", pHash, (unsigned int)k));
