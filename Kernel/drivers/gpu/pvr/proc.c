									 ...
									 )
{
    ...
    if (!psPerProc->psProcDir)
    {
		...
		{
			PVR_DPF((PVR_DBG_ERROR, "CreatePerProcessProcEntries: couldn't generate per process proc directory name \"%u\"", (unsigned int)ui32PID));
			return NULL;
		}
		else
		{
			...
			if (!psPerProc->psProcDir)
			{
				PVR_DPF((PVR_DBG_ERROR, "CreatePerProcessProcEntries: couldn't create per process proc directory /proc/%s/%u",
						PVRProcDirRoot, (unsigned int)ui32PID));
				return NULL;
			}
		}
    }
...
IMG_INT CreatePerProcessProcEntry(const IMG_CHAR * name, read_proc_t rhandler, write_proc_t whandler, IMG_VOID *data)
{
    ...
    if (!psPerProc->psProcDir)
    {
		...
		else
		{
			...
			if (!psPerProc->psProcDir)
			{
			PVR_DPF((PVR_DBG_ERROR, "CreatePerProcessProcEntries: couldn't create per process proc directory /proc/%s/%u", PVRProcDirRoot, (unsigned int)ui32PID));
...
static void ProcSeqShowVersion(struct seq_file *sfile,void* el)
{
	...
	if(el == PVR_PROC_SEQ_START_TOKEN)
	{
		seq_printf( sfile,
						"Version %s (%s,%s %s) %s\n",
						PVRVERSION_STRING,
						PVR_BUILD_TYPE, __DATE__, __TIME__, PVR_BUILD_DIR);
		return;
	}
