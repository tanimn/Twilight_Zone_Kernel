...
DestroyOffsetStruct(PKV_OFFSET_STRUCT psOffsetStruct)
{
   ...
   list_del(&psOffsetStruct->sAreaItem);
    ...
    PVR_DPF((PVR_DBG_MESSAGE, "%s: Table entry: "
             "psLinuxMemArea=0x%p, CpuPAddr=0x%08X", __FUNCTION__,
				...
				IMG_UINT32 *pui32UserVAddr)
{
    ...
    if (eError != PVRSRV_OK)
    {
	PVR_DPF((PVR_DBG_ERROR, "%s: Lookup of handle 0x%p failed", __FUNCTION__, hMHandle));
                                ...
                                IMG_UINT32 *pui32UserVAddr)
{
    ...
    PVR_DPF((PVR_DBG_ERROR, "%s: Mapping data not found for handle 0x%p (memory area 0x%p)", __FUNCTION__, hMHandle, psLinuxMemArea));
