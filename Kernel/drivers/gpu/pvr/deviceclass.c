...
PVRSRV_ERROR PVRSRVRemoveDCDeviceKM(IMG_UINT32 ui32DevIndex)
{
	...
	if (!psDeviceNode)
	{
		...
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVRemoveDCDeviceKM: requested device %u not present", (unsigned int)ui32DevIndex));
	...
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVRemoveDCDeviceKM: failed as %u Services DC API connections are still open", (unsigned int)psDCInfo->ui32RefCount));
		return PVRSRV_ERROR_GENERIC;	
	}
...
PVRSRV_ERROR PVRSRVRemoveBCDeviceKM(IMG_UINT32 ui32DevIndex)
{
	...
	if (!psDevNode)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVRemoveBCDeviceKM: requested device %u not present", (unsigned int)ui32DevIndex));
		return PVRSRV_ERROR_GENERIC;
	}
	...
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVRemoveBCDeviceKM: failed as %u Services BC API connections are still open", (unsigned int)psBCInfo->ui32RefCount));
		return PVRSRV_ERROR_GENERIC;		
	}
								   ...
								   IMG_HANDLE				*phDeviceKM)
{
	...
	if (!psDeviceNode)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVOpenDCDeviceKM: no devnode matching index %u", (unsigned int)ui32DeviceID));
		return PVRSRV_ERROR_GENERIC;
	}
	psDCInfo = (PVRSRV_DISPLAYCLASS_INFO*)psDeviceNode->pvDevice;
								   ...
								   IMG_HANDLE				*phDeviceKM)
{
	...
	if (!psDeviceNode)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVOpenBCDeviceKM: No devnode matching index %u", (unsigned int)ui32DeviceID));
		return PVRSRV_ERROR_GENERIC;
	}
								  ...
								  IMG_HANDLE *phBuffer)
{
	...
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVGetBCBufferKM: Buffer index %u out of range (%u)", (unsigned int)ui32BufferIndex, (unsigned int)psBCInfo->ui32BufferCount));
		return PVRSRV_ERROR_INVALID_PARAMS;
	}
