...
{
	PVR_DPF((PVR_DBG_MESSAGE,"WriteHWReg Base:%p, Offset: %x, Value %x", pvLinRegBaseAddr, (unsigned int)ui32Offset, (unsigned int)ui32Value));
...
PVRSRV_ERROR PVRSRVFinaliseSystem_SetPowerState_AnyCb(PVRSRV_DEVICE_NODE *psDeviceNode)
{
	...
	if (eError != PVRSRV_OK)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVFinaliseSystem: Failed PVRSRVSetDevicePowerStateKM call (device index: %u)", (unsigned int)psDeviceNode->sDevId.ui32DeviceIndex));
	}
	return eError;
}
...
PVRSRV_ERROR PVRSRVFinaliseSystem_CompatCheck_AnyCb(PVRSRV_DEVICE_NODE *psDeviceNode)
{
	...
	if (eError != PVRSRV_OK)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVFinaliseSystem: Failed PVRSRVDevInitCompatCheck call (device index: %u)", (unsigned int)psDeviceNode->sDevId.ui32DeviceIndex));
	}
	return eError;
}
...
PVRSRV_ERROR IMG_CALLCONV PVRSRVDeinitialiseDevice(IMG_UINT32 ui32DevIndex)
{
	...
	if (!psDeviceNode)
	{
		PVR_DPF((PVR_DBG_ERROR,"PVRSRVDeinitialiseDevice: requested device %u is not present", (unsigned int)ui32DevIndex));
		return PVRSRV_ERROR_GENERIC;
	}
														...
														IMG_SIZE_T *puiBufSize, IMG_BOOL bSave)
{
	...
	while (RA_GetNextLiveSegment(hArena, &sSegDetails))
	{
		...
		else
		{
			...
			PVR_DPF((PVR_DBG_MESSAGE, "PVRSRVSaveRestoreLiveSegments: Base %08x size %08x", (unsigned int)sSegDetails.sCpuPhyAddr.uiAddr, (unsigned int)sSegDetails.uiSize));
